...
{    
    public static class WorldExtensionEventHandler
    {
        ...
        public void onChunkDataLoad(ChunkDataEvent.Load event)
        {
            ...
            for(lib.world.WorldExtension extension : worldMap.get(event.world))
        ...
        public void onWorldUnLoad(WorldEvent.Unload event)
        {
            if(worldMap.containsKey(event.world))//because force closing unloads a world twice
                for(lib.world.WorldExtension extension : worldMap.remove(event.world))
                    extension.unload();
        }
        ...
        {            
            ...
            for(lib.world.WorldExtension extension : worldMap.get(event.player.worldObj))
    ...
    private static HashMap<World, lib.world.WorldExtension[]> worldMap = new HashMap<World, lib.world.WorldExtension[]>();
    
    private static void onWorldLoad(World world)
    {
        lib.world.WorldExtension[] extensions = new lib.world.WorldExtension[extensionIntialisers.size()];
        for(int i = 0; i < extensions.length; i++)
            extensions[i] = extensionIntialisers.get(i).createWorldExtension(world);
        
        worldMap.put(world, extensions);
        
        for(lib.world.WorldExtension extension : extensions)
            extension.load();
    }

    private static void createChunkExtension(World world, Chunk chunk)
    {
        lib.world.WorldExtension[] extensions = worldMap.get(world);
    ...
    {
        for(lib.world.WorldExtension extension : worldMap.get(world))
    ...
    public static lib.world.WorldExtension getWorldExtension(World world, int instantiatorID)
    {
