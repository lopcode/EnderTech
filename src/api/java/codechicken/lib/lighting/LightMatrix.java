 ...
 */
public class LightMatrix implements lib.render.IVertexModifier
{
    public int computed = 0;
    public float[][] ao = new float[13][4];
    public int[][] brightness = new int[13][4];
    public lib.vec.BlockCoord pos = new lib.vec.BlockCoord();
    ...
    public void setColour(Tessellator tess, lib.lighting.LC lc, int c)
    {
        float[] a = ao(lc.side);
    ...
    public void setBrightness(Tessellator tess, lib.lighting.LC lc)
    {
    ...
    public void applyModifiers(lib.render.CCModel m, Tessellator tess, Vector3 vec, lib.render.UV uv, Vector3 normal, int i)
    {
        ...
        if(m instanceof CCRBModel)
            ...
            lc = lib.lighting.LC.compute(v_temp.set(vec).add(-pos.x, -pos.y, -pos.z), normal);
