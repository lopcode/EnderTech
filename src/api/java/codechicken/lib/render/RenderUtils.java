...
public class RenderUtils
{
    ...
    public static void drawCuboidOutline(lib.vec.Cuboid6 c)
    {
    ...
    public static void renderFluidCuboid(lib.vec.Cuboid6 bound, Icon tex, double res)
    {
    ...
    public static Icon prepareFluidRender(FluidStack stack, int alpha)
    {
        ...
        lib.render.CCRenderState.setColour(fluid.getColor(stack) << 8 | alpha);
     ...
     */
    public static void renderFluidCuboid(FluidStack stack, lib.vec.Cuboid6 bound, double density, double res)
    {
        ...
        lib.render.CCRenderState.startDrawing(7);
        renderFluidCuboid(bound, tex, res);
        lib.render.CCRenderState.draw();
        postFluidRender();
    }
    
    public static void renderFluidGauge(FluidStack stack, lib.vec.Rectangle4i rect, double density, double res)
    {
        if(!shouldRenderFluid(stack))
            return;
    ...
    public static void renderBlock(lib.vec.Cuboid6 c, int sideMask, IFaceRenderer r)
    {
        double x1 = c.min.x;
        double x2 = c.max.x;
        double y1 = c.min.y;
    ...
    public static void renderBlock(lib.vec.Cuboid6 bounds, int sideMask, final lib.vec.Transformation t, final lib.render.IUVTransformation u, final IVertexModifier m)
    {
        renderBlock(bounds, sideMask, new IFaceRenderer(){
            boolean drawNormal = lib.render.CCRenderState.useNormals();
            ...
            public void renderFace(Vertex5[] face, int side)
            {
                ...
                for(int i = 0; i < face.length; i++)
                {
                    ...
                    {
                        if(t != null)
                            t.applyN(normal.set(lib.vec.Rotation.axes[side%6]));
                        else
                            normal = lib.vec.Rotation.axes[side%6];
