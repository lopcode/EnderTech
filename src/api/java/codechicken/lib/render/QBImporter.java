...
public class QBImporter
{
    private static class ImagePackNode
    {
        ...
        private static void nextSize(lib.vec.Rectangle4i rect, boolean square) {
            if(square) {
                rect.w <<= 1;
                rect.h <<= 1;
            }
    ...
    public static class QBQuad
    {
        ...
        public static QBQuad restore(lib.vec.Rectangle4i flat, int side, double d, QBImage img) {
            QBQuad quad = new QBQuad(side);
            quad.image = img;

            lib.vec.Transformation t = new lib.vec.Scale(-1, 1, -1).with(lib.vec.Rotation.sideOrientation(side, 0)).with(new lib.vec.Translation(new lib.vec.Vector3().setSide(side, d)));
        ...
        public lib.vec.Rectangle4i flatten() {
            lib.vec.Transformation t = lib.vec.Rotation.sideOrientation(side, 0).inverse().with(new lib.vec.Scale(-1, 0, -1));
            lib.vec.Vector3 vmin = verts[0].vec.copy().apply(t);
            lib.vec.Vector3 vmax = verts[2].vec.copy().apply(t);
            return new lib.vec.Rectangle4i((int)vmin.x, (int)vmin.z, (int)(vmax.x-vmin.x), (int)(vmax.z-vmin.z));
        }
    }
    ...
    {
        public QBMatrix mat;
        public lib.vec.CuboidCoord c;
        public int sides;

        public QBCuboid(QBMatrix mat, lib.vec.CuboidCoord c) {
            this.mat = mat;
            this.c = c;
            sides = 0;
        }
        ...
        public static boolean intersects(QBCuboid a, QBCuboid b) {
            lib.vec.CuboidCoord c = a.c;
            lib.vec.CuboidCoord d = b.c;
        ...
        public void extractQuads(List<QBQuad> quads) {
            lib.vec.Cuboid6 box = c.bounds();
            for(int s = 0; s < 6; s++)
                if((sides & 1<<s) == 0)
                    quads.add(extractQuad(s, box));
        }

        private QBQuad extractQuad(int side, lib.vec.Cuboid6 box) {
            ...
            for(int i = 0; i < 4; i++) {
                ...
                int sideU = lib.vec.Rotation.rotateSide(side, rU);
                int sideV = lib.vec.Rotation.rotateSide(side, rV);
                da[sideU>>1] = box.getSide(sideU);
                da[sideV>>1] = box.getSide(sideV);
                quad.verts[i] = new Vertex5(lib.vec.Vector3.fromAxes(da), (3-rU)/2, rV/2);
            }

            int sideU = lib.vec.Rotation.rotateSide(side, 1);
            int sideV = lib.vec.Rotation.rotateSide(side, 2);
            ...
            lib.vec.BlockCoord b = lib.vec.BlockCoord.fromAxes(ia);
            lib.vec.BlockCoord bU = lib.vec.BlockCoord.sideOffsets[sideU];
            lib.vec.BlockCoord bV = lib.vec.BlockCoord.sideOffsets[sideV];
    ...
    {
        public String name;
        public lib.vec.BlockCoord pos;
        public lib.vec.BlockCoord size;
        public int[][][] matrix;
        ...
        private boolean voxelFull(boolean[][][] solid, lib.vec.CuboidCoord c) {
            for(lib.vec.BlockCoord b : c)
        ...
        private QBCuboid expand(boolean[][][] solid, lib.vec.BlockCoord b) {
            lib.vec.CuboidCoord c = new lib.vec.CuboidCoord(b);
            solid[b.x][b.y][b.z] = false;

            for(int s = 0; s < 6; s++) {
                lib.vec.CuboidCoord slice = c.copy();
        ...
        public List<QBCuboid> rectangulate() {
            ...
            for(int x = 0; x < size.x; x++)
                for(int z = 0; z < size.z; z++)
                    for(int y = 0; y < size.y; y++)
                        if(solid[x][y][z])
                            list.add(expand(solid, new lib.vec.BlockCoord(x, y, z)));
        ...
        private void optimisePlanes(List<QBQuad> quads) {
            ...
            for(Integer key : map.keySet()) {
                ...
                for(QBQuad q : plane) {
                    ...
                    lib.vec.Rectangle4i r = q.flatten();
                    int du = r.x-rect.x;
                    int dv = r.y-rect.y;
        ...
        public CCModel buildModel(List<QBQuad> quads, BufferedImage img, boolean scaleMC) {
            ...
            m.apply(new lib.vec.Translation(pos.x, pos.y, pos.z));
            if(scaleMC)
                m.apply(new lib.vec.Scale(1/16D));
            m.computeNormals();
            return m;
        }
    ...
    public static QBModel loadQB(InputStream input) throws IOException {
        ...
        for(int i = 0; i < m.matrices.length; i++) {
            ...
            mat.size = new lib.vec.BlockCoord(readTni(din), readTni(din), readTni(din));
            mat.pos = new lib.vec.BlockCoord(readTni(din), readTni(din), readTni(din));
